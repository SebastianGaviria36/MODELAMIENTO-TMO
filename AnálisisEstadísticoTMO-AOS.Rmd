...
ggplot(proportions, aes(x = GAP, y = Proportion, fill = Status)) + 
...
```{r}
plot1 <- ggplot(prop.table(table(Datos130$`ESQUEMA NIVEL 1`)) %>% as.data.frame(), aes(x = Var1, y = Freq)) + 
  geom_bar(stat = "identity", fill = "steelblue") +
  geom_text(aes(label=Freq %>% round(2)), vjust=1.6, color="white",
            position = position_dodge(0.9), size=3.5)+ 
  labs(x = "Esquema Nivel 1", 
       y = "Proporción", 
       title = "Proporción de pacientes por esquema nivel 1") +
  theme_minimal()+
  theme(plot.title = element_text(hjust = 0.5), 
        axis.title.x = element_text(face="bold",  size = 12),
        axis.title.y = element_text(face="bold",  size = 12),
...
plot2 <- ggplot(prop.table(table(Datos130$SEXO)) %>% as.data.frame(), aes(x = Var1, y = Freq)) + 
  ...
  theme(plot.title = element_text(hjust = 0.5), 
        ...
        axis.title.y = element_text(face="bold",  size = 12),
        legend.title = element_text(face="bold", size = 10)) 
plot3 <- ggplot(Datos130, aes(x = EDAD)) +
...
plot4 <- ggplot(Datos130, aes(x = IMC)) +
...
- Aproximadamente el 31% de los pacientes ingresan al tratamiento mediante el esquema TCC, mientras que el 69% restante ingresan directamente por fonoaudiología.

- El 65% de los pacientes son femeninos, diferencia significativa con la muestra masculina.
...
```{r}
ggplot(data.frame(X1 = c(rep("Salida", 126), rep("Basal",126)),
...
```{r, fig.height=3}
plot21 <- ggplot(prop.table(table(Datos130$`SINTOMAS BASALES`)) %>% as.data.frame(), aes(x = Var1, y = Freq, fill = Var1)) + 
  geom_bar(stat = "identity") +
  geom_text(aes(label=Freq %>% round(2)), vjust=1.6, color="white",
            position = position_dodge(0.9), size=3.5)+ 
  labs(x = "", 
       y = "Proporción", 
       title = "Síntomas Basales",
       fill = "Síntoma") +
  theme_minimal()+
  theme(plot.title = element_text(hjust = 0.5), 
        axis.title.x = element_text(face="bold",  size = 12),
...
plot22 <- ggplot(prop.table(table(Datos130$`SINTOMA PRINCIPAL`)) %>% as.data.frame(), aes(x = Var1, y = Freq, fill = Var1)) + 
  geom_bar(stat = "identity") +
  geom_text(aes(label=Freq %>% round(2)), vjust=1.6, color="white",
            position = position_dodge(0.9), size=3.5)+ 
  labs(x = "", 
...
- Del primer gráfico de barras se aprecia que el síntoma basal principal es por diferencia el cansancio crónico, marcado como primero el 40% de las veces. Los siguientes síntomas más recurrentes en este apartado son la cefalea matutina y el despertarse con ahogo.

- Por otro lado, del segundo gráfico se aprecia algo distinto, y es que en este caso, el 26% de los pacientes definieron el ronquido como su síntoma principal. Otro síntoma destacable en este caso es la dificultad para mantener el sueño, puesto el 14% de las veces como síntoma principal.
...
```{r}
ggplot(prop.table(table(Datos130$`TIPO EXAMEN DE SUEÑO`)) %>% as.data.frame(), aes(x = Var1, y = Freq, fill = Var1)) + 
  geom_bar(stat = "identity") +
  geom_text(aes(label=Freq %>% round(2)), vjust=1.6, color="white",
            position = position_dodge(0.9), size=3.5)+ 
  labs(x = "", 
...
- La mayoría de pacientes que ingresan al tratamiento se les practica una Polisomnografía como examen del sueño, abarcando el 87% de los casos.
...
```{r, fig.height=8}
plot31 <- ggplot(Datos130, aes(x = `IAH BASAL`)) +
  ...
  theme(plot.title = element_text(hjust = 0.5), 
        ...
        axis.title.y = element_text(face="bold",  size = 12),
        legend.title = element_text(face="bold", size = 10))
plot32 <- ggplot(Datos130, aes(x = `INDICE DE RONQUIDO/h`)) +
  geom_density(color="orange", fill="darkorange") + 
  labs(x = "", 
       y = "Densidad", 
  ...
  theme(plot.title = element_text(hjust = 0.5), 
        ...
        axis.title.y = element_text(face="bold",  size = 12),
        legend.title = element_text(face="bold", size = 10))
plot33 <- ggplot(Datos130, aes(x = `% SpO2`)) +
  geom_density(color="orange", fill="darkorange") + 
  labs(x = "", 
       y = "Densidad", 
       title = "% SpO2") +
...
plot34 <- ggplot(Datos130, aes(x = `MÍNIMAS DE O2`)) +
  geom_density(color="orange", fill="darkorange") + 
  labs(x = "", 
       y = "Densidad", 
       title = "Minimas de o2") +
...
plot35 <- ggplot(Datos130, aes(x = `T 90`)) +
  geom_density(color="orange", fill="darkorange") + 
  labs(x = "", 
       y = "Densidad", 
       title = "T 90") +
...
ggarrange(plot31,plot32,plot33,plot34,plot35, ncol = 2, nrow = 3)
```
...
- La curva de densidad para el índice de ronquido se nota un poco más extendida sobre su rango, donde un poco menos de la mitad de los pacientes registran menos de 200 en este índice de ronquido. Nuevamente se aprecia una cola hacia la derecha de la distribución muestral de esta variable, lo que indica presencia de valores extremos por encima de 600.
...
```{r}
ggplot(prop.table(table(unlist(strsplit(Datos130$`CUAL (ES)`, ", ")))) %>% as.data.frame(), aes(x = Var1, y = Freq, fill = Var1)) + 
  geom_bar(stat = "identity") +
  geom_text(aes(label=Freq %>% round(2)), vjust=1.6, color="white",
            position = position_dodge(0.9), size=3.5)+ 
  labs(x = "", 
...
ggplot(prop.table(table(Datos130$OVAS)) %>% as.data.frame(), aes(x = Var1, y = Freq, fill = Var1)) + 
  geom_bar(stat = "identity") +
  geom_text(aes(label=Freq %>% round(2)), vjust=1.6, color="white",
            position = position_dodge(0.9), size=3.5)+ 
  labs(x = "", 
...
- Del primer gráfico se abserva que, el 74% de los pacientes no ha tenido una cirugía en sus vías aéreas superiores. El 11% ha tenido una Amigdalectomía, el 8% una Turbinoplastia y el 6% una Septoplastia. 

- Del segundo se concluye que el 65% de los pacientes no poseen obstrucciones de la vía aérea superior.
...
```{r}
plot41 <- ggplot(prop.table(table(Datos130$`LENG RETRAÍDA`)) %>% as.data.frame(), aes(x = Var1, y = Freq, fill = Var1)) + 
  geom_bar(stat = "identity") +
  geom_text(aes(label=Freq %>% round(2)), vjust=1.6, color="white",
            position = position_dodge(0.9), size=3.5)+ 
  labs(x = "", 
...
plot42 <- ggplot(prop.table(table(Datos130$`UVULA ALARGADA`)) %>% as.data.frame(), aes(x = Var1, y = Freq, fill = Var1)) + 
  geom_bar(stat = "identity") +
  geom_text(aes(label=Freq %>% round(2)), vjust=1.6, color="white",
            position = position_dodge(0.9), size=3.5)+ 
  labs(x = "", 
...
plot43 <- ggplot(prop.table(table(Datos130$`VELO DESCENDIDO`)) %>% as.data.frame(), aes(x = Var1, y = Freq, fill = Var1)) + 
  geom_bar(stat = "identity") +
  geom_text(aes(label=Freq %>% round(2)), vjust=1.6, color="white",
            position = position_dodge(0.9), size=3.5)+ 
  labs(x = "", 
...
plot44 <- ggplot(prop.table(table(Datos130$`PILARES ERITEMATOSOS`)) %>% as.data.frame(), aes(x = Var1, y = Freq, fill = Var1)) + 
  geom_bar(stat = "identity") +
  geom_text(aes(label=Freq %>% round(2)), vjust=1.6, color="white",
            position = position_dodge(0.9), size=3.5)+ 
  labs(x = "", 
...
ggarrange(plot41,plot42,plot43,plot44)
```

- Del primero se nota que el 67% de los pacientes que terminaron su tratamiento poseen la condición de lengua retraída.

- Similarmente, el 79% de los pacientes poseen una uvula alargada, mientras que en un 7% de las personas no se logra concluír sobre esta condición.
...
- De manera casi idéntica, el 96% de los pacientes registra pilares eritematosos.
...
De esta tabla se aprecia que prácticamente todos los pacientes que poseen velo descendido, poseen además pilares eritematosos.
...
```{r}
plot51 <- ggplot(prop.table(table(Datos130$CO_Cardiovasculares)) %>% as.data.frame(), aes(x = Var1, y = Freq, fill = Var1)) + 
  geom_bar(stat = "identity") +
  geom_text(aes(label=Freq %>% round(2)), vjust=1.6, color="white",
            position = position_dodge(0.9), size=3.5)+ 
  labs(x = "", 
...
plot52 <- ggplot(prop.table(table(Datos130$CO_Endocrino_metabólicas)) %>% as.data.frame(), aes(x = Var1, y = Freq, fill = Var1)) + 
  geom_bar(stat = "identity") +
  geom_text(aes(label=Freq %>% round(2)), vjust=1.6, color="white",
            position = position_dodge(0.9), size=3.5)+ 
  labs(x = "", 
...
plot53 <- ggplot(prop.table(table(Datos130$CO_Psiquiátricas)) %>% as.data.frame(), aes(x = Var1, y = Freq, fill = Var1)) + 
  geom_bar(stat = "identity") +
  geom_text(aes(label=Freq %>% round(2)), vjust=1.6, color="white",
            position = position_dodge(0.9), size=3.5)+ 
  labs(x = "", 
...
plot54 <- ggplot(prop.table(table(Datos130$CO_Pulmonares)) %>% as.data.frame(), aes(x = Var1, y = Freq, fill = Var1)) + 
  geom_bar(stat = "identity") +
  geom_text(aes(label=Freq %>% round(2)), vjust=1.6, color="white",
            position = position_dodge(0.9), size=3.5)+ 
  labs(x = "", 
...
ggarrange(plot51,plot52,plot53,plot54)
```
...
- La proporción de pacientes con comorbilidades psiquiátricas o pulmonares es baja, con 15% y 17% de los pacientes respectivamente.
...
```{r, fig.height=10}
plot61 <- ggplot(prop.table(table(Datos130$Línea_media_desviada)) %>% as.data.frame(), aes(x = Var1, y = Freq, fill = Var1)) + 
  geom_bar(stat = "identity") +
  geom_text(aes(label=Freq %>% round(2)), vjust=1.6, color="white",
            position = position_dodge(0.9), size=3.5)+ 
  labs(x = "", 
...
plot62 <- ggplot(prop.table(table(Datos130$Mordida_borde_a_borde)) %>% as.data.frame(), aes(x = Var1, y = Freq, fill = Var1)) + 
  geom_bar(stat = "identity") +
  geom_text(aes(label=Freq %>% round(2)), vjust=1.6, color="white",
            position = position_dodge(0.9), size=3.5)+ 
  labs(x = "", 
...
plot63 <- ggplot(prop.table(table(Datos130$Mordida_abierta_anterior)) %>% as.data.frame(), aes(x = Var1, y = Freq, fill = Var1)) + 
  geom_bar(stat = "identity") +
  geom_text(aes(label=Freq %>% round(2)), vjust=1.6, color="white",
            position = position_dodge(0.9), size=3.5)+ 
  labs(x = "", 
...
plot64 <- ggplot(prop.table(table(Datos130$Mordida_cruzada)) %>% as.data.frame(), aes(x = Var1, y = Freq, fill = Var1)) + 
  geom_bar(stat = "identity") +
  geom_text(aes(label=Freq %>% round(2)), vjust=1.6, color="white",
            position = position_dodge(0.9), size=3.5)+ 
  labs(x = "", 
...
plot65 <- ggplot(prop.table(table(Datos130$Maloclusión_clase_II)) %>% as.data.frame(), aes(x = Var1, y = Freq, fill = Var1)) + 
  geom_bar(stat = "identity") +
  geom_text(aes(label=Freq %>% round(2)), vjust=1.6, color="white",
            position = position_dodge(0.9), size=3.5)+ 
  labs(x = "", 
...
plot66 <- ggplot(prop.table(table(Datos130$Maloclusión_clase_III)) %>% as.data.frame(), aes(x = Var1, y = Freq, fill = Var1)) + 
  geom_bar(stat = "identity") +
  geom_text(aes(label=Freq %>% round(2)), vjust=1.6, color="white",
            position = position_dodge(0.9), size=3.5)+ 
  labs(x = "", 
...
ggarrange(plot61,plot62,plot63,plot64,plot65,plot66, ncol=2,nrow=3)
```

- De las condiciones en la mordida de los pacientes bajo tratamiento, la línea media desviada es la más frecuente con un 57% de los pacientes que la poseen.

- Condiciones como mordida borde aborde, mordida abierta anterior y mordida cruzada son poco usuales entre los pacientes. En todos los casos, menos del 10% de los pacientes tuvieron alguna de estas condiciones.

- El 16% y 13% de los pacientes reportaron maloclusión clase II y maloclusión clase III respectivamente.
...
```{r}
ggplot(prop.table(table(Datos130$`MAN BOSTEZO - VELO`)) %>% as.data.frame(), aes(x = Var1, y = Freq, fill = Var1)) + 
  geom_bar(stat = "identity") +
  geom_text(aes(label=Freq %>% round(2)), vjust=1.6, color="white",
            position = position_dodge(0.9), size=3.5)+ 
  labs(x = "", 
...
- De este gráfico se puede apreciar que el 80% de los pacientes ascienden el velo en la maniobra de bostezo, con un 43% para aquellos que lo hacen de forma reducida, y un 37% para aquellos que en la maniobra de bostezo ascienden el velo totalmente. Un 16% de los pacientes poseen un dorso lingual lo suficientemente alto para impedir la conclusión en esta maniobra. 


```{r}
ggplot(prop.table(table(Datos130$`MAN BOSTEZO - ÚVULA`)) %>% as.data.frame(), aes(x = Var1, y = Freq, fill = Var1)) + 
  geom_bar(stat = "identity") +
  geom_text(aes(label=Freq %>% round(2)), vjust=1.6, color="white",
            position = position_dodge(0.9), size=3.5)+ 
  labs(x = "", 
...
- Continuando con la misma maniobra pero observando la úvula, en el gráfico se logra apreciar que 1/4 de los pacientes no contrae la úvula en la maniobra de bostezo, proporción completamente análoga a aquellos pacientes que poseen un dorso lingual que imposibilita la observación del resultado. En este caso la mayoría de los pacientes contraen la úvula totalmente, sin embargo la diferencia con los demás grupos es reducida.


```{r}
ggplot(prop.table(table(Datos130$`MAN. FONEMA - VELO`)) %>% as.data.frame(), aes(x = Var1, y = Freq, fill = Var1)) + 
  geom_bar(stat = "identity") +
  geom_text(aes(label=Freq %>% round(2)), vjust=1.6, color="white",
            position = position_dodge(0.9), size=3.5)+ 
  labs(x = "", 
...
- Pasando a la maniobra de fonema, según el gráfico se puede observa que casi un 60% de los pacientes asciende el velo de manera reducida en esta prueba. En este caso los pacientes que no ascienden el velo son minoría con un 10% y los demás grupos empatan sus proporciones con cerca de un 15%.


```{r}
ggplot(prop.table(table(Datos130$`MAN FONEMA - ÚVULA`)) %>% as.data.frame(), aes(x = Var1, y = Freq, fill = Var1)) + 
  geom_bar(stat = "identity") +
  geom_text(aes(label=Freq %>% round(2)), vjust=1.6, color="white",
            position = position_dodge(0.9), size=3.5)+ 
  labs(x = "", 
...
- Finalmente, observando la úvula en la maniobra de fonema, se puede notar que una mayoría de los pacientes no la contraen en este ejercicio con un 35% del total de los participantes. Es destacable en este caso que hay una gran proporción (29%) de pacientes con un dorso lingual tan alto como para no observar el resultado de la práctica. 
